...
<?php
    include __DIR__ . '/../_templates/notificacao-lista-item.php';

    $tabJsPath = __DIR__ . '/../../../public/assets/javascript/utils/tab.js';
    $tabJsVer = is_file($tabJsPath) ? (string) filemtime($tabJsPath) : (string) time();
?>

<script type="text/javascript" src="<?= obterURL('/assets/javascript/utils/tab.js?v=' . $tabJsVer) ?>"></script>
...
<script type="text/javascript">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (csrfToken && window.$ && typeof $.ajaxSetup === 'function') {
            $.ajaxSetup({
                headers: {
        ...
        const tab = new Tab('#main-notificacoes', {
            prefixo: 'notificacao',
            ...
            estadoVazio: {
                ...
                descricao: 'Tente ajustar os filtros ou a sua busca.'
            },
        ...
        if (buttonMarcarTodasLidas) {
            buttonMarcarTodasLidas.addEventListener('click', function() {
                $.ajax({
                    url: '/notificacoes/todas/ler',
                    method: 'POST',
                    data: { token_csrf: csrfToken },
                    success: function(response) {
                        if (response.status === 'sucesso') {
                            if (tab && typeof tab.invalidarCache === 'function') {
                                tab.invalidarCache();
                            }
