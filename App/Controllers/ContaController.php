    ...
    public function salvarInformacoesPessoais(Request $request): void {
        ...
        try {
            ...
            if ($imagemPerfil && $imagemPerfil['error'] !== UPLOAD_ERR_NO_FILE) {
                ...
                // Caminho físico absoluto para armazenamento das imagens de perfil dentro de `public/`
                ...
                if (!is_dir($diretorioImagensPerfil) && !mkdir($diretorioImagensPerfil, 0775, true) && !is_dir($diretorioImagensPerfil)) {
                    throw new Exception('Não foi possível preparar o diretório para salvar a imagem de perfil.');
                }
                ...
                $caminhoDestino = $diretorioImagensPerfil . $novoNomeArquivo;

                // Move o arquivo para o diretório de destino
                if (move_uploaded_file($caminhoTemporario, $caminhoDestino)) {

                    // Se o usuário já possuia uma imagem de perfil, exclua-a
                    if ($usuarioAutenticado->obterCaminhoFoto() && file_exists($diretorioImagensPerfil . $usuarioAutenticado->obterCaminhoFoto())) {
                        unlink($diretorioImagensPerfil . $usuarioAutenticado->obterCaminhoFoto());
                    }
            ...
            if (($usuarioAutenticado->obterCaminhoFoto() != null) && $removerFoto) {
                ...
                if (file_exists($diretorioImagensPerfil . $usuarioAutenticado->obterCaminhoFoto())) {
                    unlink($diretorioImagensPerfil . $usuarioAutenticado->obterCaminhoFoto());
                }
