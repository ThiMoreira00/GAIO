 ...
 * @description Classe-auxiliar para gerenciamento de mensagens flash via cookies (stateless).
...
/**
 * Classe NotificadorHelper
 *
 * Responsável por gerenciar e exibir mensagens flash para o usuário.
 * Utiliza cookies JSON em vez de sessão para compatibilidade com autenticação JWT.
 *
 * @package App\Helper
 */
class NotificadorHelper
{
    ...
    /**
     * Nome do cookie para mensagens flash
     */
    private const COOKIE_FLASH = 'gaio_flash';
    ...
    /**
     * Mensagens pendentes acumuladas na requisição atual
     * @var array
     */
    private array $mensagensPendentes = [];
    ...
    {
        // Carrega mensagens existentes do cookie
        $this->mensagensPendentes = $this->lerCookieFlash();
    }
    ...
    public function adicionar(string $tipo, string $mensagem, ?string $titulo = null): void
    {
        ...
        // Atualiza o cookie com as mensagens acumuladas
        $this->gravarCookieFlash($this->mensagensPendentes);
    }
    ...
    protected function limparMensagens(): void
    {
        ...
        unset($_COOKIE[self::COOKIE_FLASH]);
    }
    ...
    {
        return $this->mensagensPendentes;
    }
        ...
        return sprintf(
            ...
            <div class="flex-shrink-0">
                <svg class="%s" viewBox="0 0 20 20" aria-hidden="true">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="%s" stroke-width="1.5" fill="currentColor"></path>
                </svg>
            </div>
            <div class="ml-3">
                <h3 class="%s">%s</h3>
    ...
    // --- MÉTODOS AUXILIARES DE COOKIE ---

    /**
     * Lê as mensagens flash do cookie
     *
     * @return array
     */
    private function lerCookieFlash(): array
    {
        if (!isset($_COOKIE[self::COOKIE_FLASH]) || empty($_COOKIE[self::COOKIE_FLASH])) {
            return [];
        }

        $mensagens = json_decode($_COOKIE[self::COOKIE_FLASH], true);
    ...
    /**
     * Grava as mensagens flash no cookie
     *
     * @param array $mensagens
     * @return void
     */
    private function gravarCookieFlash(array $mensagens): void
    {
        $seguro = ($_ENV['SISTEMA_AMBIENTE'] ?? 'desenvolvimento') === 'producao';
        $valor = json_encode($mensagens, JSON_UNESCAPED_UNICODE);

        setcookie(self::COOKIE_FLASH, $valor, [
            'expires' => time() + 60,
            'path' => '/',
            'secure' => $seguro,
            'httponly' => false,
        ...
        // Atualiza o superglobal para a requisição atual
        $_COOKIE[self::COOKIE_FLASH] = $valor;
    }
}
