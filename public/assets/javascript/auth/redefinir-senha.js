document.addEventListener('DOMContentLoaded', function () {
    ...
    togglePasswordButtons.forEach(button => {
        button.addEventListener('click', (ev) => {
            ...
            const input = container ? container.querySelector('input[type="password"], input') : null;
            console.debug('[redefinir-senha] toggle clicked', { button, container, input });
            if (!input) return;
            input.type = input.type === 'password' ? 'text' : 'password';
            console.debug('[redefinir-senha] input type now', input.type);
        });
    });
