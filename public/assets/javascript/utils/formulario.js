class Formulario {
    constructor(formId, options = {}) {
        // Suporta chamada com um único objeto { formId: '...', onSuccess: ..., ... }
        if (typeof formId === 'object' && formId !== null) {
            options = formId;
            formId = options.formId;
        }

        const { onBeforeSubmit, onSuccess, onError, onComplete, notificador } = options;
        
        // Converte ID para seletor CSS se não for um seletor válido
        if (formId && !formId.startsWith('#') && !formId.startsWith('.') && !formId.includes('[')) {
            formId = '#' + formId;
        }
